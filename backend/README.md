...
## Deploying to Render (frontend + backend together)

Because the Express server already serves the static website files, you only need **one Render service** for everything.
...
4. In the **Environment** tab, fill in the secrets marked `sync: false`:
   - `MONGODB_URI` — your Atlas connection string
   - `FRONTEND_URL` — your Render app URL, e.g. `https://visionworks.onrender.com`
   - `GOOGLE_CALLBACK_URL` — `https://visionworks.onrender.com/api/auth/google/callback`
   - OAuth keys if using Google / Apple
5. Click **Deploy**

`SESSION_SECRET` is auto-generated by Render.

No `API_BASE` changes needed in the frontend — relative URLs work because the same server serves both.
...
- [ ] Update OAuth callback URLs to your Render domain
